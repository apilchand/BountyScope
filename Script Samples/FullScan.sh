...
# Generated: 2025-08-30T05:58:44.342Z
# Selected Tools: 39
#
...
# --- Phase 07: Content Discovery & Directory Fuzzing ---
run_phase_07_content_fuzzing() {
	log_info "Phase 07: Running Content Fuzzing..."
	log_sub_info "Running ffuf Fuzzer..."
	ffuf -w /opt/SecLists/Discovery/Web-Content/common.txt -u https://apilchand.com.np/FUZZ -mc 200,301,302 -o $RECON_DIR/scans/ffuf_root.json
	log_sub_info "Running Gobuster Directory..."
	gobuster dir -u https://apilchand.com.np -w /opt/SecLists/Discovery/Web-Content/common.txt -o $RECON_DIR/scans/gobuster_root.txt
}

# --- Phase 08: JavaScript Analysis & API Discovery ---
run_phase_08_js_&_api_analysis() {
	log_info "Phase 08: Running JS & API Analysis..."
	log_sub_info "Running subjs..."
	cat $RECON_DIR/urls/final.txt | grep -iE '\\.js($|\\?)' | subjs | anew $RECON_DIR/urls/js_files.txt
	log_sub_info "Running LinkFinder..."
	cat $RECON_DIR/urls/js_files.txt | xargs -I % linkfinder -i % -o cli | anew $RECON_DIR/urls/linkfinder.txt
	log_sub_info "Running SecretFinder..."
	cat $RECON_DIR/urls/js_files.txt | xargs -I % secretfinder -i % -o cli | anew $RECON_DIR/scans/secrets.txt
}
...
# --- Phase 10: Cloud Asset & Storage Discovery ---
run_phase_10_cloud_discovery() {
	log_info "Phase 10: Running Cloud Discovery..."
	log_sub_info "Running S3Scanner..."
	# Example: s3scanner -bucket-file buckets.txt -dump
	log_sub_info "Running Cloud_enum..."
	cloud_enum -k apilchand.com.np -l $RECON_DIR/scans/cloud_enum.txt
}

# --- Phase 11: Git Repository & Source Code Analysis ---
run_phase_11_source_code_secrets() {
	log_info "Phase 11: Running Source Code Secrets..."
	log_sub_info "Running TruffleHog..."
	trufflehog github --org=apilchand.com.np --json > $RECON_DIR/scans/trufflehog.json
	log_sub_info "Running GitLeaks..."
	gitleaks detect --source . -r $RECON_DIR/scans/gitleaks.json
}
...
# --- Phase 14: CMS & Framework Security Assessment ---
run_phase_14_cms_scanning() {
	log_info "Phase 14: Running CMS Scanning..."
	log_sub_info "Running WPScan..."
	wpscan --url https://apilchand.com.np --enumerate ap,at,u -o $RECON_DIR/scans/wpscan.json -f json
	log_sub_info "Running Nikto Web Scanner..."
	nikto -h https://apilchand.com.np -ssl -output $RECON_DIR/scans/nikto.txt
}
